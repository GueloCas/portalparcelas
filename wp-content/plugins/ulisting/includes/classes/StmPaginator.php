...
class StmPaginator {
	 ...
	 */
	public function __construct($totalItems, $itemsPerPage, $currentPage, $urlPattern = '', $options = array())
	{
		...
		$this->maxPagesToShow        = (isset($options['maxPagesToShow'])) ? $options['maxPagesToShow'] : 10;
		$this->updateNumPages();
	}
	...
	public function toHtml()
	{
		...
		$html = '<ul class="'.$this->options['class'].'">';
		if ($this->getPrevUrl()) {
			$html .= '<li class="'.$this->options['item_class'].'"><a class="'.$this->options['link_class'].'" href="' . htmlspecialchars($this->getPrevUrl()) . '">'. $this->previousText .'</a></li>';
		}
		foreach ($this->getPages() as $page) {
			if ($page['url']) {
				$html .= '<li' . ($page['isCurrent'] ? ' class="active"' : '') . '><a class="'.$this->options['link_class'].'" href="' . htmlspecialchars($page['url']) . '">' . htmlspecialchars($page['num']) . '</a></li>';
			} else {
				$html .= '<li class="disabled '.$this->options['item_class'].'"><span  class="'.$this->options['link_class'].'" >' . htmlspecialchars($page['num']) . '</span></li>';
			}
		}
		if ($this->getNextUrl()) {
