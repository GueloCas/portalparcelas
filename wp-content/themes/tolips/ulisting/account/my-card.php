...
<div id="stripe-my-card">
		<div class="panel-custom p-t-30 p-b-30">
			<stripe-card-component ref="stripe_card" inline-template  v-bind:publishable_key="publishable_key" v-on:set-stripe-token-emit="set_stripe_token">
				...
				<hr>
			<button class="btn-medium btn-theme" v-if="!loading" @click="add_card" ><?php echo esc_html__('Add Card', "tolips")?></button>
		...
		<div class="panel-custom p-t-30 p-b-30">
			<div v-for="card in cards">
				<div class="stm-row">
					...
					<div class="stm-col-3"> <?php echo esc_html__("Expired:", "tolips") ?> {{card.exp_month}}/{{card.exp_year}} </div>
						...
						<vue-ulist-dropdown v-if="!card.loading" inline-template :visible="card.visible" @clickout="card.visible = false">
							<div>
								...
								<div class="ulist-dropdown-content" v-bind:class="{ 'ulist-dropdown-show': visible }">
									<span @click="delete_card(card)" class="item"><?php esc_html_e('Delete', "tolips"); ?></span>
									<span v-if="!card.default" @click="make_default(card)" class="item"><?php esc_html_e('Make default', "tolips"); ?></span>
								</div>
							</div>
						</vue-ulist-dropdown>
					</div>
